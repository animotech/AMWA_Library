...
- [ライブラリ説明](#ライブラリ説明)
  - [通信基本API](#通信基本api)
  - [AutoUDP API](#autoudp-api)
  - [シリアル helper API](#シリアル-helper-api)
- [デモ説明](#デモ説明)
- [使用ガイド](#使用ガイド)
- [著者](#著者)
- [ライセンス情報](#ライセンス情報)
...
* **bool socket_exists(int id)**<br>
指定した socket ID が有効か確認する関数です。(AT+SLISTコマンド)<br>
引数<br>
・int id : ソケットID<br>
返り値<br>
・bool : 指定IDのソケットが存在する場合 true
...
補足: 内部ブリッジコンテキストの同時使用上限は 4 インスタンスです。5 個以上の `AMWA` オブジェクトを同時利用する用途は非推奨です。<br>

* **void at_receive_begin()**<br>
AT受信側の内部状態（FIFO/待機状態）を初期化する関数です。<br>
シリアルブリッジ処理を開始する前に 1 回呼び出します。<br>

* **void at_receive_poll()**<br>
AT側 UART から受信データを読み取り、内部FIFOへ取り込む関数です。<br>
loop 内で継続的に呼び出して受信を進めます。<br>

* **size_t at_output_block(Stream &out, size_t maxChunk = 256)**<br>
CR/LF終端、または内部の強制フラッシュ条件でまとまったAT応答を `out` へ出力する関数です。<br>
引数<br>
・Stream &out : 出力先ストリーム（例: Serial）<br>
・size_t maxChunk : 1 回に出力する最大バイト数<br>
返り値<br>
・size_t : 出力したバイト数

* **bool at_receive_waiting_response()**<br>
AT 応答待ち中かどうかを判定する関数です。<br>
true の間は、LOG 側から AT 側への送信を抑制する用途で使用します。<br>

* **void at_send_byte(uint8_t b)**<br>
AT側 UART へ 1 byte 送信する関数です。<br>

* **size_t at_send_bytes(const uint8_t *data, size_t len)**<br>
AT側 UART へ複数 byte を送信する関数です。<br>
引数<br>
・const uint8_t *data : 送信データ先頭ポインタ<br>
・size_t len : 送信バイト数<br>
返り値<br>
・size_t : 実際に送信したバイト数

* **void log_receive_line_begin(size_t reserveLen = 128)**<br>
LOG入力1行バッファを初期化する関数です。<br>

* **bool log_receive_line(String &outLine, size_t maxLen = 128, int budget = 64)**<br>
Serial Monitor 側入力を CRLF 区切りで 1 行取り出す関数です。<br>
引数<br>
・String &outLine : 取得した行文字列の出力先<br>
・size_t maxLen : 最大受信長（超過時は破棄）<br>
・int budget : 1回呼び出しで処理する最大文字数<br>
返り値<br>
・bool : 1行取得できた場合 true
...
AMWA-01がTCPサーバになり、TCPクライアントから受信したデータをエコーします。
