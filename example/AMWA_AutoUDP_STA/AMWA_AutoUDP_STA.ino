...
#define AUTOUDP_BAUD   38400             // AutoUDP 中の UART baud。AP 側と必ず揃える。
...
void setup() {
  ...
  while (true) {
    // STA モード初回 boot は AP 検索→association→DHCP まで時間がかかるため、
    // detect_boot_state は 40 秒待つ。AutoUDP 設定済みの再 boot で connect
    // できれば数秒で "AutoUDP" が出る。
